...
- Another alternative to this approach is to use custom wrapper components that just return `props.children` and we wrap our JSX elements with those components. But in that wrapper component we are still returning only one element which is `props.children`.
## React Fragments
